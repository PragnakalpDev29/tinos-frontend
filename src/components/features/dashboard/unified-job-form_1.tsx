    ...
    return (
        <div className="max-w-6xl mx-auto space-y-8">
            ...
            <form onSubmit={handleSubmit}>
                <div className="bg-[#08333D] rounded-xl shadow-sm border border-[#466F78]/30 p-8 space-y-6">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-[#466F78] text-sm">Fill in the job details to start processing.</p>
                        </div>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label className="block text-sm font-semibold text-[#08333D] mb-2">Job Name (Optional)</label>
                            <input type="text" name="job_name" value={formData.job_name} onChange={handleInputChange} className="w-full px-4 py-3 border border-[#466F78]/50 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent outline-none" placeholder="e.g. run-1" />
                            <p className="mt-1.5 text-xs text-[#466F78]">Defaults to {jobNamePlaceholder}</p>
                        </div>
                        <div>
                            <label className="block text-sm font-semibold text-[#08333D] mb-2">S3 RNA BAM Path <span className="text-red-500">*</span></label>
                            <input type="text" name="s3_rna_bam" value={formData.s3_rna_bam} onChange={handleInputChange} required className="w-full px-4 py-3 border border-[#466F78]/50 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent outline-none" placeholder="s3://..." />
                        </div>
                    </div>

                    <div className="relative my-8">
                        <div className="absolute inset-0 flex items-center"><div className="w-full border-t border-[#466F78]/30"></div></div>
                        <div className="relative flex justify-center"><span className="bg-[#08333D] px-4 text-sm font-bold text-slate-400 uppercase">Or</span></div>
                    </div>
                    ...
                    <div className="bg-gradient-to-br from-teal-900/30 to-emerald-50 border-2 border-teal-200 rounded-xl p-6 space-y-5">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-[#466F78]">Using Tus resumable uploads for better reliability.</p>
                            </div>
                        </div>

                        <div className="bg-[#08333D] rounded-lg p-5 border border-[#466F78]/50 shadow-sm transition-all focus-within:ring-2 focus-within:ring-teal-500">
                            <label className="block text-sm font-semibold text-[#08333D] mb-3">Select Files (Min 2 required)</label>
                            <input ref={fileInputRef} type="file" multiple accept=".bam" onChange={handleFileChange} disabled={uploading} className="block w-full text-sm text-[#466F78] file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-bold file:bg-[#466F78]/30 file:text-[#466F78] hover:file:bg-teal-800 cursor-pointer p-4 border-2 border-dashed border-[#466F78]/50 rounded-lg hover:border-teal-400" />
                            <p className="mt-2 text-xs text-[#466F78] flex items-center gap-1"><Info className="w-3 h-3" /> Resumable upload used for all file sizes.</p>
                        </div>

                        {uploading && (
                            <div className="bg-[#08333D] rounded-lg p-4 border border-[#466F78]/50 shadow-sm">
                                <div className="flex justify-between items-center mb-2">
                                    <span className="text-sm font-semibold text-[#08333D]">Overall Progress</span>
                        ...
                        {files && files.length > 0 && (
                            <div className="mt-5 space-y-2">
                                {Array.from(files).map((file, index) => {
                                    ...
                                    return (
                                        <div key={index} className="flex items-center justify-between p-3 bg-[#08333D] rounded-lg border border-[#466F78]/30">
                                            <div className="flex items-center gap-3 flex-1 min-w-0">
                                                <Database className="w-4 h-4 text-[#466F78] shrink-0" />
                                                <div className="flex-1 min-w-0">
                                                    <p className="text-sm font-semibold text-[#08333D] truncate">{file.name}</p>
                                                    {status && (
                                                        <div className="w-full bg-[#08333D] h-1 rounded-full mt-1 overflow-hidden">
                                            ...
                                            <div className="flex gap-1 ml-4 shadow-sm">
                                                ...
                                                {status?.status === 'paused' && <button type="button" onClick={() => resumeUpload(file.name)} className="p-1 px-2 rounded-md bg-green-900 text-green-400 hover:bg-green-200 transition-colors"><Play className="w-3.5 h-3.5" /></button>}
